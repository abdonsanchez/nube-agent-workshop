...
export class Agent {
    ...
    _abortSignal = this._abortController.signal;
    _printer;
     ...
     */
    _extractCheckpointResume(args) {
        if (typeof args !== 'object' || args === null || Array.isArray(args) || !('checkpointResume' in args)) {
            return undefined;
        }
        if (!this._checkpointing) {
            throw new CheckpointError('Received a checkpointResume block but the agent was created with checkpointing: false. ' +
                'Pass checkpointing: true when constructing the Agent.');
        }
        const payload = args.checkpointResume;
     ...
     */
    _redactLastMessage(redactMessage) {
            ...
            if (lastMessage && lastMessage.role === 'user') {
                ...
                for (const block of lastMessage.content) {
                    if (block.type === 'toolResultBlock') {
                        ...
                        redactedContent.push(new ToolResultBlock({
                            toolUseId: block.toolUseId,
                            status: block.status,
