...
export class Tracer {
    ...
    startModelInvokeSpan(options) {
        ...
        this._traceState.currentModel = new AgentTrace('stream_messages', this._traceState.currentCycle ? { parent: this._traceState.currentCycle } : undefined);
        try {
    ...
    startToolCallSpan(options) {
        ...
        const toolTrace = new AgentTrace(`Tool: ${tool.name}`, this._traceState.currentCycle ? { parent: this._traceState.currentCycle } : undefined);
    ...
    startMultiAgentSpan(options) {
        ...
        try {
            ...
            if (input)
                attributes['gen_ai.agent.input'] = JSON.stringify(input, jsonReplacer);
            const mergedAttributes = { ...attributes, ...this._traceAttributes, ...traceAttributes };
            const span = this._startSpan({ name: spanName, attributes: mergedAttributes, spanKind: SpanKind.INTERNAL });
     ...
     */
    endMultiAgentSpan(span, options = {}) {
        ...
        try {
            ...
            if (options.duration !== undefined)
                attributes['gen_ai.agent.execution_time'] = options.duration;
            if (options.usage)
                this._setUsageAttributes(attributes, options.usage);
     ...
     */
    endNodeSpan(span, options = {}) {
        if (!span)
            return;
        try {
            const attributes = {};
            if (options.status)
                attributes['gen_ai.agent.status'] = options.status;
            if (options.duration !== undefined)
                attributes['gen_ai.agent.execution_time'] = options.duration;
            if (options.usage)
                this._setUsageAttributes(attributes, options.usage);
     ...
     */
    endMemorySearchSpan(span, options = {}) {
        if (!span)
            return;
        try {
     ...
     */
    startMemoryInjectSpan(options = {}) {
        try {
            const attributes = this._getCommonAttributes('memory.inject');
            if (options.maxEntries !== undefined)
                attributes['memory.max_entries'] = options.maxEntries;
     ...
     */
    endMemoryInjectSpan(span, options) {
        if (!span)
            return;
        try {
            const attributes = {
     ...
     */
    startMemoryExtractSpan(options) {
        try {
            ...
            if (options.extractor !== undefined)
                attributes['memory.extractor'] = options.extractor;
            ...
            const validAgentSpanContext = options.agentSpanContext && isSpanContextValid(options.agentSpanContext) ? options.agentSpanContext : undefined;
            ...
            const links = validAgentSpanContext ? [{ context: validAgentSpanContext }] : undefined;
            return this._startSpan({
                name: 'memory.extract',
                attributes,
                spanKind: SpanKind.INTERNAL,
     ...
     */
    endMemoryExtractSpan(span, options = {}) {
        if (!span)
            return;
        try {
            const attributes = {};
            if (options.entryCount !== undefined)
                attributes['memory.entry.count'] = options.entryCount;
            this._endSpan(span, attributes, options.error);
        }
        catch (err) {
     ...
     */
    _addSystemPromptEvent(span, systemPrompt) {
        if (systemPrompt === undefined)
            return;
        if (this._useLatestConventions) {
     ...
     */
    static _mapSystemPromptToOtelParts(systemPrompt) {
