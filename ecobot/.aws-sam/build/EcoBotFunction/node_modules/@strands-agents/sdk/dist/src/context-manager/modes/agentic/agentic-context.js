...
import { generateSummary, adjustSplitPointForToolPairs, findValidTrimPoint, matchesMessageType, } from '../../../conversation-manager/compression/context-compression.js';
...
const messageTypeSchema = z
    ...
    .optional()
    .describe('Filter which messages to target. "tools" targets only tool use/result messages, ' +
 ...
 */
function collectPreserved(messages, rangeEnd, filter) {
    const eligible = [];
    const preserved = [];
...
export const summarizeContextTool = tool({
    ...
    description: 'Compress the oldest messages in your conversation into a concise summary to free up context space. ' +
    ...
    inputSchema: z.object({
        ...
        summaryRatio: z
            ...
            .optional()
            .describe(`Fraction of the oldest messages to fold into the summary (${MIN_SUMMARY_RATIO}–${MAX_SUMMARY_RATIO}). Defaults to ${DEFAULT_SUMMARY_RATIO}.`),
...
export const truncateContextTool = tool({
    ...
    description: 'Drop the oldest messages from your conversation history entirely to free up context space. ' +
    ...
    inputSchema: z.object({
        keepRecent: z
            ...
            .min(MIN_MESSAGES_FOR_COMPRESSION)
            .optional()
            .describe(`Number of most recent messages to keep. Everything older (and unpinned) is dropped. Defaults to ${DEFAULT_KEEP_RECENT_MESSAGES}.`),
        messageType: messageTypeSchema,
    }),
...
export function createTokenUsageMiddleware(model) {
...
export const pinContextTool = tool({
    name: 'pin_context',
    description: 'Pin or unpin messages in the conversation history. ' +
    ...
    inputSchema: z.object({
        select: z
            ...
            .describe('Which messages to target. "last_turn" for the current exchange, a number for the last N messages, or an array of indices.'),
        filter: z
            .enum(['user', 'assistant', 'tools'])
            .optional()
            .describe('Narrow the selection to only messages matching this filter. ' +
    ...
    callback: ({ select, filter, action }, context) => {
        ...
        if (select === 'last_turn') {
            ...
            while (i >= 0) {
                ...
                const msg = messages[i];
                if (msg.role === 'user' && msg.content.some((b) => b.type === 'textBlock'))
                    break;
                i--;
            }
        }
        ...
        const targetIndices = filter
            ? candidateIndices.filter((i) => {
                const msg = messages[i];
                if (filter === 'user')
                    return msg.role === 'user' && msg.content.some((b) => b.type === 'textBlock');
                if (filter === 'assistant')
                    return msg.role === 'assistant' && msg.content.some((b) => b.type === 'textBlock');
                if (filter === 'tools')
...
//# sourceMappingURL=agentic-context.js.map