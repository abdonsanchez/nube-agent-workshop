 ...
 * is no overlap.
 */
function tokenOverlapScore(queryTokens, content) {
    let score = 0;
...
export class TestMemoryStore {
    ...
    async search(query, options) {
        ...
        scored.sort((left, right) => right.score - left.score || right.record.createdAt.localeCompare(left.record.createdAt));
    ...
    async add(content, metadata) {
        ...
        const run = this._writeChain.then(async () => {
            ...
            const record = { id: uuidv7(), content, createdAt: new Date().toISOString() };
    ...
    async _read() {
        ...
        for (const record of parsedBlob) {
            if (record === null ||
                ...
                throw new Error(`TestMemoryStore: invalid backing file ${key}: ` +
