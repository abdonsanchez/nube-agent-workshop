...
import { FileStorage, InMemoryStorage as LegacyInMemoryStorage } from './storage.js';
...
function isOffloaderStorage(storage) {
...
function frameContent(content, contentType) {
...
async function storeContent(storage, key, content, contentType) {
    if (isOffloaderStorage(storage))
        return storage.store(key, content, contentType);
    const ct = contentType ?? 'application/octet-stream';
    await storage.write(key, frameContent(content, ct));
    return key;
}
async function retrieveContent(storage, reference) {
    if (isOffloaderStorage(storage))
...
const retrievalInputSchema = z.object({
    ...
    context_lines: z
        ...
        .min(0)
        .optional()
        .describe('Lines before AND after each match (like grep -C). Default: 5. Without pattern/line_range, returns first N lines.'),
});
...
export class ContextOffloader {
    ...
    static _DEFAULT_EVICT_AFTER_CYCLES = 20;
    _sandboxableStorage;
    _storage;
    _maxResultTokens;
    _previewTokens;
    ...
    constructor(config) {
        ...
        const evictAfterCycles = config.evictAfterCycles === undefined ? ContextOffloader._DEFAULT_EVICT_AFTER_CYCLES : config.evictAfterCycles;
    ...
    _storageForAgent(agent) {
        if (!this._sandboxableStorage && !(this._storage instanceof FileStorage))
    ...
    _storageForToolContext(context) {
        if (!this._sandboxableStorage && !(this._storage instanceof FileStorage))
            return this._storage;
        if (!context) {
    ...
    _createRetrievalTool() {
        ...
        return tool({
            ...
            description: 'When a tool result was too large to keep in context, it was stored externally and replaced with a preview and a reference. ' +
    ...
    async _storeBlock(storage, block, key) {
