 ...
 * @internal
 */
export async function resolveServerConfigs(config, defaults) {
    ...
    for (const [name, server] of Object.entries(servers)) {
        ...
        try {
            ...
            if (!type)
                throw new Error('Server config must include either "command" (stdio) or "url" (http)');
            let clientConfig;
            switch (type) {
                case 'stdio':
...
function buildStdioConfig(server) {
    if (!server.command)
    ...
    if (server.args)
        opts.args = server.args.map(interpolateEnv);
    if (server.env)
        opts.env = { ...getDefaultEnvironment(), ...interpolateRecord(server.env) };
    if (server.cwd)
        opts.cwd = interpolateEnv(server.cwd);
    return { transport: new StdioClientTransport(opts) };
}
function buildHttpConfig(server) {
    if (!server.url)
        throw new Error('Streamable HTTP transport requires "url" field');
    const config = { url: interpolateEnv(server.url) };
    if (server.headers)
        config.headers = interpolateRecord(server.headers);
    if (server.auth) {
        config.auth = {
...
function buildSseConfig(server) {
    if (!server.url)
    ...
    return {
        transport: new SSEClientTransport(new URL(interpolateEnv(server.url)), headers ? { requestInit: { headers } } : undefined),
    };
}
function baseOptions(name, server, defaults) {
    ...
    const opts = { ...defaults, applicationName: defaults?.applicationName ?? name };
    if (server.continueOnError != null)
        opts.continueOnError = server.continueOnError;
    if (server.tasksConfig != null)
        opts.tasksConfig = server.tasksConfig;
    return opts;
}
/**
...
async function loadServersObject(config) {
    ...
    if (!servers || typeof servers !== 'object' || Array.isArray(servers)) {
        throw new Error('MCP config must be a JSON object mapping server names to configs, e.g. { "my-server": { "command": "node" } }');
    }
    return servers;
}
