...
import { ModelContentBlockDeltaEvent, ModelContentBlockStartEvent, ModelContentBlockStopEvent, ModelMessageStartEvent, ModelMessageStopEvent, ModelMetadataEvent, ModelRedactionEvent, } from './streaming.js';
 ...
 */
export function resolveConfigMetadata(config, modelId) {
...
export class Model {
    ...
    async *streamAggregated(messages, options) {
        try {
            ...
            let redactionMessage = undefined;
            let toolInputParseError = undefined;
            for await (const event_data of this.stream(messages, options)) {
                ...
                switch (event.type) {
                    ...
                    case 'modelMessageStopEvent':
                        ...
                        if (messageRole) {
                            const filtered = contentBlocks.filter((block) => !(block instanceof TextBlock && block.text.trim() === ''));
            ...
            if (!stoppedMessage || !finalStopReason) {
                ...
                throw new ModelError('Stream ended without completing a message', toolInputParseError ? { cause: toolInputParseError } : undefined);
            }
            ...
            if (finalStopReason === 'maxTokens') {
                throw new MaxTokensError('Model reached maximum token limit. This is an unrecoverable state that requires intervention.', stoppedMessage);
            }
